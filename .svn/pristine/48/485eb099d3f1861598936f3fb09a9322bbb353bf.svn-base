...
function memberModify() {
	...
	f.action = "<c:url value="/member_modify"/>";
	f.submit();
}
</script>
</head>
...
<form id="modifyBtn" name="f" class="form" method="post">
	<h3 class="mb-5" style="text-align: center; font-size: 10px; font-weight:900; color: #706FFF;">회원정보 수정</h3>
	<label for="id">
		<p>아이디</p>
		<span>${loginHost.MId}</span>
	</label>
	
	<label for="password">
		<p>비밀번호</p>	
		<input class="join" type="password" id="passwd" name="mPw"><br>
	</label>
	<div>
		<label><span style="color: red; text-align: center;">** 비밀번호를 변경하지 않을 경우 입력하지 마세요. **</span></label>
	</div>
	<div class="mb-5">
	...
	<label for="name">
		<p>이름</p>	
		<input class="join" type="text" id="name" name="mName" value="${loginHost.MName}">
	</label>
	<div class="mb-5">
	...
	<label for="email">
		<p>이메일</p>	
		<input class="join" type="email" id="email" name="mEmail" value="${loginHost.MEmail}">
	</label>
	<div class="mb-5">
	...
	<label for="phone">
		<p>휴대폰 번호</p>	
		<input class="join" type="text" id="phone" name="mPhone" value="${loginHost.MPhone}">
	</label>
	<div class="mb-5">
		<span id="phoneValidMsg" class="msg">휴대폰 번호는 01로 시작하며, 3~4 자리의 숫자로만 입력 가능합니다.</span>	
	</div>	
	<button type="submit" id="modifyBtn" onclick="memberModify();">수정</button>
</form>
